...
# Loading the model
model = torch.hub.load('ultralytics/yolov5', 'custom', path='/home/birdscarekku/dev/yolov5/runs/train/exp/weights/best.pt')  # local model

cap = cv2.VideoCapture("/home/birdscarekku/dev/target-detector-yolov5-main/Pictures/"+str(i)+".png")
            ...
            if center_x is not None and center_y is not None:
                ...
                if name == 'Pigeon':
                    ...
                    if count < number_of_photos:
                        cv2.imwrite("Detected Photos/detected" + str(count) + ".png", frame_detected)
                    # Playing the alarm
                    if not pygame.mixer.music.get_busy():
                        pygame.mixer.music.play()
                        alarm_playing = True
                    cv2.putText(frame, "Target", (center_x, center_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    cv2.putText(frame, "Pigeon Detected", (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
